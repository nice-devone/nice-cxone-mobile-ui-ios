...
struct MessageInputView: View, Themed {
    ...
    private var isVoiceRecordVisible: Bool {
        message.isEmpty && attachments.isEmpty && attachmentRestrictions.areVoiceMessagesEnabled
    }
        ...
        HStack(alignment: audioRecorder.state == .idle ? .bottom : .center, spacing: 0) {
            if audioRecorder.state != .idle {
                ...
                if #available(iOS 16.0, *) {
                    ...
                    LegacyMultilineTextField(attributedText: self.attributedMessage, isEditing: self.$isEditing)
                }
            }
