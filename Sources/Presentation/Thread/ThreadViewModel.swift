...
extension ThreadViewModel: CXoneChatDelegate {
    ...
    func onThreadUpdated(_ updatedThread: CXoneChatSDK.ChatThread) {
        // Handle only if the thread is the one currently displayed one
        guard thread == nil || (thread?.id == updatedThread.id) else {
            LogManager.trace("Ignoring thread update - the thread is not the currently oned one")
            return
        }
...
private extension ThreadViewModel {
    ...
    func updateThread(with updatedThread: CXoneChatUI.ChatThread?) throws {
        ...
        self.isInputEnabled = updatedThread != nil && updatedThread?.state != .closed
...
extension ThreadViewModel {
    ...
    var menu: MenuBuilder {
        MenuBuilder()
            .add(
                if: isInputEnabled && canEditCustomFields,
